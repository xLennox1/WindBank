(() => {
  ...
  .section{scroll-margin-top:20px!important}.section-head{border-left:4px solid #5ebf65;padding-left:12px}
  .wb-category-shell{display:none!important}
  .wb-mobile-bottom{display:none}
  @media(max-width:720px){
    .wrap{width:calc(100% - 20px)!important}.nav{height:60px!important}.admin-btn{padding:8px 9px!important;font-size:11px}.brand{font-size:17px}.mark{width:34px;height:34px}
  ...
  function setup(){
    ...
    if(!document.querySelector('.wb-mobile-bottom')){
      ...
      bottom.setAttribute('aria-label','Mobile Kategorien');
      bottom.innerHTML='<a data-target="shop" href="#shop"><span class="ico">⛏️</span>Shop</a><a data-target="basis-versicherung" href="#basis-versicherung"><span class="ico">🛡️</span>Versicherung</a><a data-target="kredit" href="#kredit"><span class="ico">💰</span>Kredit</a><a data-target="reviews" href="#reviews"><span class="ico">⭐</span>Reviews</a>';
      document.body.appendChild(bottom);
      const spacer=document.createElement('div');spacer.className='wb-mobile-space';document.body.appendChild(spacer);
    }
    const links=[...document.querySelectorAll('.wb-mobile-bottom a')];
    ...
    const sections=['shop','basis-versicherung','kredit','reviews'].map(id=>document.getElementById(id)).filter(Boolean);
