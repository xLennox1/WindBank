(() => {
  ...
  body.wb-category-mode .hero,body.wb-category-mode footer{display:none!important}body.wb-category-mode .wb-hidden-section{display:none!important}body.wb-category-mode .wb-active-section{display:block!important;padding-top:35px!important;min-height:calc(100vh - 80px)}.wb-mobile-bottom{display:none}
  ...
  function uniqueSections(){
    const set=new Set();
    const add=e=>{if(e&&!set.has(e)){set.add(e)}};
    document.querySelectorAll('main > .section, .section').forEach(add);
    document.querySelectorAll('#grid,#reviewsGrid,.loan').forEach(e=>{const s=e.closest('.section')||e.parentElement;add(s)});
    return [...set];
  }
  function textOf(el){return (el?.textContent||'').replace(/\s+/g,' ').trim().toLowerCase()}
  function nearestSection(el){return el?.closest('.section')||el?.parentElement||null}
  function sectionFor(type){
    const sections=uniqueSections();
    if(type==='shop') return document.getElementById('grid')?.closest('.section')||sections.find(s=>s.querySelector('#grid'))||null;
    if(type==='reviews') return document.getElementById('reviewsGrid')?.closest('.section')||sections.find(s=>s.querySelector('#reviewsGrid'))||null;
    if(type==='kredit'){
      const loan=document.querySelector('.loan,[class*="loan" i],[id*="kredit" i],[id*="loan" i]');
      if(loan)return nearestSection(loan);
      return sections.find(s=>textOf(s).includes('kredit')||textOf(s).includes('darlehen')||textOf(s).includes('zins')||s.querySelector('.rate'))||null;
    }
    if(type==='versicherung'){
      const direct=[...document.querySelectorAll('[id*="versicherung" i],[id*="insurance" i],[class*="versicherung" i],[class*="insurance" i]')].find(e=>!e.closest('.modal'));
      if(direct)return nearestSection(direct);
      return sections.find(s=>textOf(s).includes('versicherung')||textOf(s).includes('basis-schutz')||textOf(s).includes('base insurance')||textOf(s).includes('basen'))||null;
    }
    return null;
  }
  function showCategory(type){
    ...
    uniqueSections().forEach(s=>{s.classList.remove('wb-active-section');s.classList.add('wb-hidden-section')});
    if(section){section.classList.remove('wb-hidden-section');section.classList.add('wb-active-section');window.scrollTo({top:0,behavior:'smooth'})}
  ...
  function installNav(){
    ...
    const bottom=document.createElement('nav');bottom.className='wb-mobile-bottom';bottom.setAttribute('aria-label','Kategorien');
    bottom.innerHTML='<a data-target="shop" href="#shop"><span class="ico">⛏️</span>Shop</a><a data-target="versicherung" href="#versicherung"><span class="ico">🛡️</span>Versicherung</a><a data-target="kredit" href="#kredit"><span class="ico">💰</span>Kredit</a><a data-target="reviews" href="#reviews"><span class="ico">⭐</span>Reviews</a>';
    ...
    bottom.addEventListener('click',e=>{const a=e.target.closest('a');if(!a)return;e.preventDefault();showCategory(a.dataset.target)});
  }
  function applyHash(){const h=(location.hash||'').slice(1);showCategory(['shop','versicherung','kredit','reviews'].includes(h)?h:'shop')}
  function setup(){
    addStyle();installNav();
    if(typeof wbInstallBaseInsurance==='function')wbInstallBaseInsurance();
    setTimeout(applyHash,300);setTimeout(applyHash,1000);setTimeout(applyHash,2000);
