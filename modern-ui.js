(() => {
  ...
  function sectionFor(type){
    ...
    const aliases={shop:['shop','markt','store'],versicherung:['versicherung','basis-versicherung','basisversicherung','insurance'],kredit:['kredit','credit','darlehen'],reviews:['reviews','rezension','rezensionen','bewertungen']};
    const wanted=aliases[type]||[];
    let found=sections.find(s=>wanted.includes((s.id||'').toLowerCase()));
    if(found)return found;
    return sections.find(s=>wanted.some(w=>(s.textContent||'').toLowerCase().includes(w)))||null;
  }
  ...
  function setup(){
    ...
    showCategory(['shop','versicherung','kredit','reviews'].includes(initial)?initial:'shop');
  }
