(() => {
  ...
  function sectionFor(type){
    const wanted={shop:['shop','markt'],versicherung:['versicherung von basen','basis-schutz'],kredit:['kredite','kredit'],reviews:['rezensionen','rezension','reviews','bewertungen']}[type]||[];
    return getSections().find(s=>{
      const headings=[...s.querySelectorAll('h1,h2,h3')].map(h=>(h.textContent||'').trim().toLowerCase());
      return headings.some(h=>wanted.some(w=>h===w||h.includes(w)));
    })||null;
  }
  ...
  function setup(){
    ...
    const apply=()=>showCategory(['shop','versicherung','kredit','reviews'].includes((location.hash||'').slice(1))?(location.hash||'').slice(1):'shop');
    setTimeout(apply,1000);setTimeout(apply,1800);
  }
