...
export default async function handler(req,res){res.setHeader('Cache-Control','no-store');if(!authorized(req))return res.status(401).json({ok:false,error:'Nicht autorisiert.'});try{
 ...
 if(action==='delete'){await sb('listings?id=eq.'+encodeURIComponent(req.body.id),{method:'DELETE',headers:{Prefer:'return=minimal'}});return res.status(200).json({ok:true});}
 ...
 if(action==='updateMain'){const b=req.body;const row={name:String(b.name||'').trim(),price:Number(b.price),stock:Number(b.stock),description:String(b.description||''),image_url:String(b.image_url||''),status:Number(b.stock)===0?'sold_out':'approved'};await sb('listings?id=eq.'+encodeURIComponent(b.id),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify(row)});return res.status(200).json({ok:true});}
